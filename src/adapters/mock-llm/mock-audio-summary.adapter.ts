...
export class MockAudioSummaryAdapter implements AudioSummaryPort {
    ...
    async summarize(transcription: Transcription): Promise<string> {
        this.logger.debug(`Simulating summary generation for transcription ID: ${transcription.whatsappAudioId}`);

        // Simulate a delay
        await new Promise(resolve => setTimeout(resolve, 1000));

        return `[MOCK SUMMARY] The user sent an audio message containing ${transcription.segments.length} segments. The main point discussed was: "${transcription.fullText.substring(0, 30)}..."`;
    }
}
